...
async function fallbackQdnRequest<T>(request: QdnRequest): Promise<T> {
  switch (request.action.toUpperCase()) {
    ...
    case 'GET_SELECTED_ACCOUNT':
      throw new Error('Selected account context is only available inside Qortium Home.');
    default:
...
export async function getBridgeState(): Promise<BridgeState> {
  ...
  try {
    isUsingPublicNode = Boolean(
      await qdnRequest<unknown>({ action: 'IS_USING_PUBLIC_NODE' }),
    );
  } catch {
    // Browser fallback is always the local read-only development node.
  }
